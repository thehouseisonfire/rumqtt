...
impl Publish {
    ...
    #[doc(hidden)]
    pub fn write_prevalidated(&self, buffer: &mut BytesMut) -> Result<usize, Error> {
...
fn validate_publish_topic_name(topic: &[u8]) -> Result<(), Error> {
    ...
    let topic = core::str::from_utf8(topic).map_err(|source| Error::TopicNotUtf8 { source })?;
    if topic.contains('\0') {
...
impl fmt::Debug for Publish {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let topic = core::str::from_utf8(&self.topic).unwrap_or("<invalid utf-8>");
        write!(
            f,
...
mod test {
    ...
    fn publish_topic_with_bom_round_trips_without_stripping() {
        ...
        assert_eq!(core::str::from_utf8(&decoded.topic).unwrap(), topic);
    }
}
