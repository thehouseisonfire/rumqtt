//! Finite MQTT 3.1.1 publish/subscribe smoke test for deployment recipes.
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let port = env::var("RUMQTTC_RECIPE_PORT")
        ...
        .unwrap_or(Ok(1883))?;
    let client_id = format!("rumqttc-v4-recipe-smoke-{}", process::id());
    let topic = format!("rumqttc/recipes/smoke/{client_id}");
    let payload = b"mqtt-v4-recipe-ok";
    ...
    .map_err(|_| {
        io::Error::new(
            ...
            "timed out waiting for MQTT 3.1.1 SUBACK",
        )
    })??;
    ...
    .map_err(|_| {
        io::Error::new(
            io::ErrorKind::TimedOut,
            "timed out waiting for MQTT 3.1.1 echo",
        )
    })??;

    println!("MQTT 3.1.1 broker recipe smoke test passed");
    Ok(())
}
