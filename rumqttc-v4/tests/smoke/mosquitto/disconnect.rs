...
fn smoke_dir() -> PathBuf {
    std::env::temp_dir().join(format!(
        "rumqttc-v4-mosquitto-disconnect-smoke-{}-{}",
        std::process::id(),
        SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system clock should be after unix epoch")
...
async fn start_will_observer(
    ...
    let options = MqttOptions::new(
        format!("rumqttc-v4-smoke-{port}-observer"),
        ("127.0.0.1", port),
    );
...
async fn mixed_qos_graceful_disconnect_suppresses_will() {
    ...
    let port = broker.port;
    let will_topic = format!("rumqttc/v4/smoke/{port}/will");
    ...
    let mut options = MqttOptions::new(
        format!("rumqttc-v4-smoke-{port}-publisher"),
        ("127.0.0.1", port),
    );
    options.set_keep_alive(5).set_last_will(will);
    ...
    assert!(
        logs.contains(&format!(
            "Received DISCONNECT from rumqttc-v4-smoke-{port}-publisher"
        )),
...
async fn graceful_disconnect_with_unsent_backlog_suppresses_will() {
    ...
    let port = broker.port;
    let will_topic = format!("rumqttc/v4/smoke/{port}/backlog/will");
    ...
    let will = LastWill::new(will_topic, "will-published", QoS::AtLeastOnce, false);
    let mut options = MqttOptions::new(
        format!("rumqttc-v4-smoke-{port}-backlog-publisher"),
        ("127.0.0.1", port),
    );
    options
        .set_keep_alive(5)
    ...
    assert!(
        logs.contains(&format!(
            "Received DISCONNECT from rumqttc-v4-smoke-{port}-backlog-publisher"
        )),
