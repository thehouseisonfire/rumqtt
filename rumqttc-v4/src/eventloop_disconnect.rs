...
impl EventLoop {
    ...
    pub async fn poll(&mut self) -> Result<Event, ConnectionError> {
        ...
        let resumable = !self.mqtt_options.clean_session();
...
mod tests {
    ...
    async fn publish_flush_timeout_preserves_only_replayable_shutdowns() {
        for qos in [
            ...
            let mut options = MqttOptions::new("flush-timeout", "localhost");
            ...
            // The peer never reads, so flushing the publish fills the transport.
            let (stream, _peer) = tokio::io::duplex(1);
            eventloop.network = Some(Network::new(stream, 4096, 4096));
            client
                .try_publish("replay", "payload", crate::PublishOptions::new(qos))
                .unwrap();
            let notice = client
                .try_disconnect_after_queued_with_timeout(Duration::from_secs(10))
                .unwrap();
            let deadline = eventloop.requests_rx.gate().snapshot().1;
            let error = loop {
                if let Err(error) = eventloop.poll().await {
                    break error;
                }
            };
            if qos == crate::QoS::AtMostOnce {
                assert!(matches!(error, ConnectionError::OrderedDisconnect(_)));
                assert!(notice.wait_async().await.is_err());
                assert!(eventloop.disconnect_complete);
            } else {
                assert!(matches!(error, ConnectionError::FlushTimeout));
                assert!(!eventloop.disconnect_complete);
                assert_eq!(eventloop.shutdown_phase, crate::ShutdownPhase::Approaching);
                assert_eq!(eventloop.requests_rx.gate().snapshot().1, deadline);
                assert!(matches!(
                    eventloop.pending.front().unwrap().request,
                    Request::Publish(_)
                ));
                assert!(matches!(
                    eventloop.pending.back().unwrap().request,
    ...
    #[tokio::test]
    async fn disconnect_flush_timeout_is_terminal_for_persistent_session() {
        let mut options = MqttOptions::new("disconnect-flush-timeout", "localhost");
        options.set_clean_session(false);
        let (client, mut eventloop) = crate::AsyncClient::builder(options).build();
        eventloop.network_options.set_connection_timeout(1);
        ...
        assert!(matches!(
            ...
            Err(ConnectionError::OrderedDisconnect(
                crate::DisconnectNoticeError::Transport(_)
            ))
        ));
        assert!(matches!(
            notice.wait_async().await,
            Err(crate::DisconnectNoticeError::Transport(error))
                if matches!(&*error, ConnectionError::FlushTimeout)
        ));
        assert!(eventloop.disconnect_complete);
    ...
    async fn injected_post_fence_tracked_request_is_discarded_without_execution() {
        ...
        eventloop
            .handle_request(envelope, &mut should_flush, &mut qos0)
            .await
            .unwrap();
        assert!(!should_flush);
        assert!(matches!(
