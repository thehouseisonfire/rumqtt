...
assert_release_channel() {
  ...
  if [[ "$version" == *-* ]]; then
    echo "error: ${version} is a prerelease; use ./scripts/publish-crates-alpha.sh" >&2
    exit 1
  fi
}
...
maybe_push_and_release() {
  ...
  echo
  printf 'Push branch and tags, then create the GitHub release now? [y/N]: '
  read -r reply
  if [[ "$reply" == "y" || "$reply" == "Y" ]]; then
    ...
    echo "Then create the GitHub release from tag rumqttc-next-${version}."
  fi
}
