# MQTT 5.0 — LLM Validation Corpus (Requirements + Required Context)

This corpus is optimized for systematic validation of an MQTT 5.0 client implementation. It removes front matter, legal notices, acknowledgements, most examples, and other fluff, while preserving the material an LLM validator needs: conformance scope, packet formats, fixed-header flags, data encodings, Packet Identifier rules, payload rules, property and reason-code registries, topic/subscription rules, error-handling semantics, WebSocket rules, and all numbered MQTT 5.0 mandatory normative statements.

Source file parsed: `Pasted markdown.md`. Extracted numbered requirements: **251**. Supplemental validation-critical constraints retained: **205**.

## 1. Scope for validating an MQTT 5.0 client

A client validator should check both sides of behavior:

- **Outbound conformance:** every packet the client sends has the correct packet type, fixed-header flags, Remaining Length, field order, Packet Identifier presence, properties, payload, and allowed reason codes.
- **Inbound handling:** when the client receives packets from a server, it must apply MQTT 5.0 rules for acknowledgement, session state, flow control, topic aliases, errors, DISCONNECT, AUTH, PINGRESP, retained messages, and WebSocket framing if applicable.
- **Conformance basis:** a conformant MQTT Client must send Control Packets matching Chapters 2 and 3, satisfy applicable MUST-level requirements in Chapters 1, 2, 3, 4, 6, and 7 except those only applying to the Server, support at least one ordered/lossless/bidirectional byte-stream transport, and not require extensions outside the specification to interoperate.
- **Security chapter:** Chapter 5 is non-normative; it is intentionally omitted except where error handling references Disallowed Unicode code points.

## 2. Data representation and generic decoding rules

- RFC 2119 keywords apply unless text is marked non-normative.
- Bits are numbered 7 to 0. Bit 7 is most significant; bit 0 is least significant.
- **Two Byte Integer:** unsigned 16-bit big-endian, MSB then LSB.
- **Four Byte Integer:** unsigned 32-bit big-endian, highest-order byte first.
- **UTF-8 Encoded String:** Two Byte Integer byte length followed by that many UTF-8 bytes. Maximum 65,535 bytes. Must be well-formed UTF-8, must not contain U+0000, must not encode surrogate code points U+D800 through U+DFFF, and `0xEF 0xBB 0xBF` must be treated as U+FEFF rather than stripped.
- **Binary Data:** Two Byte Integer byte length followed by that many bytes. Maximum 65,535 bytes.
- **UTF-8 String Pair:** two consecutive UTF-8 Encoded Strings, used for name/value pairs; both strings must satisfy UTF-8 Encoded String rules.
- **Variable Byte Integer:** 1 to 4 bytes, base-128, 7 data bits per byte, high bit is continuation. Encoded value must use the minimum number of bytes necessary. Maximum value is 268,435,455.

| Encoded bytes | Value range |
|---:|---:|
| 1 | 0 to 127 |
| 2 | 128 to 16,383 |
...
## 3. MQTT Control Packet structure

Every MQTT Control Packet has: Fixed Header; optional Variable Header; optional Payload, in that order. Fixed Header byte 1 uses bits 7-4 for packet type and bits 3-0 for packet-type flags. Remaining Length begins at byte 2 and is a Variable Byte Integer for the number of bytes after the Remaining Length field.

### 3.1 Packet types and fixed-header flags

| Type | Packet | Direction | Required flags bits 3-0 |
|---:|---|---|---|
| 0 | Reserved | Forbidden | `n/a` |
| 1 | CONNECT | Client → Server | `0000` |
| 2 | CONNACK | Server → Client | `0000` |
| 3 | PUBLISH | Client ↔ Server | `DUP QoS QoS RETAIN` |
| 4 | PUBACK | Client ↔ Server | `0000` |
| 5 | PUBREC | Client ↔ Server | `0000` |
| 6 | PUBREL | Client ↔ Server | `0010` |
| 7 | PUBCOMP | Client ↔ Server | `0000` |
...
| 14 | DISCONNECT | Client ↔ Server | `0000` |
| 15 | AUTH | Client ↔ Server | `0000` |

Reserved fixed-header bits must exactly match the listed flag pattern. A wrong reserved flag pattern is a Malformed Packet / Protocol Error as specified for the packet type.

### 3.2 Packet Identifier presence

Packet Identifier is a non-zero Two Byte Integer when present. Client and Server maintain independent Packet Identifier spaces. Client packet IDs for PUBLISH QoS>0, SUBSCRIBE, and UNSUBSCRIBE form one unified set for the Session; a Packet Identifier cannot be used by more than one command at the same time.

| Packet | Packet Identifier present? |
|---|---|
| CONNECT | No |
| CONNACK | No |
| PUBLISH | Yes if QoS > 0; no if QoS 0 |
| PUBACK | Yes |
| PUBREC | Yes |
| PUBREL | Yes |
| PUBCOMP | Yes |
...
A property is encoded as a Variable Byte Integer identifier followed by a value of the specified data type. A property identifier that is not valid for the packet type, or a value not encoded with the specified type, is a Malformed Packet. Property Length is a Variable Byte Integer and must be zero when no properties are present.

| Identifier | Property | Type | Valid in |
|---:|---|---|---|
...
Reason Codes are one-byte unsigned values. Values below `0x80` indicate successful completion; values `0x80` or greater indicate failure. CONNACK, PUBACK, PUBREC, PUBREL, PUBCOMP, DISCONNECT, and AUTH carry a single Reason Code in the Variable Header. SUBACK and UNSUBACK carry lists of Reason Codes in the Payload.
...
## 6. Packet-specific validation context

### CONNECT

- Fixed header: type `1`, flags `0000`.
- Variable Header fields, in order: Protocol Name, Protocol Version, Connect Flags, Keep Alive, Properties.
- Protocol Name is UTF-8 string `MQTT`; Protocol Version for MQTT 5.0 is `0x05`.
- Connect Flags byte: bit 7 User Name Flag; bit 6 Password Flag; bit 5 Will Retain; bits 4-3 Will QoS; bit 2 Will Flag; bit 1 Clean Start; bit 0 reserved and must be 0.
- Payload fields, if present, must be in order: Client Identifier, Will Properties, Will Topic, Will Payload, User Name, Password.
- ClientID is first and required; zero-length ClientID can be accepted only if the Server assigns a unique ClientID and returns Assigned Client Identifier in CONNACK.
- If Will Flag is 0: Will QoS and Will Retain must be 0 and Will Properties/Topic/Payload must be absent. If Will Flag is 1: Will Properties, Will Topic, and Will Payload are present; Will QoS is 0, 1, or 2, never 3.
- Password may be sent without User Name in MQTT 5.0; validate each flag independently.
- CONNECT properties include Session Expiry Interval, Receive Maximum, Maximum Packet Size, Topic Alias Maximum, Request Response Information, Request Problem Information, User Property, Authentication Method, and Authentication Data. Property-specific duplicate/value errors are Protocol Errors.

### CONNACK

- Fixed header: type `2`, flags `0000`. Variable Header fields: Connect Acknowledge Flags, Connect Reason Code, Properties.
- Connect Acknowledge Flags bits 7-1 are reserved and must be 0; bit 0 is Session Present.
- Server must send CONNACK with Success before sending any packet other than AUTH and must not send more than one CONNACK on a Network Connection.
- If CONNACK Reason Code is `0x80` or greater, Server must close the Network Connection and Session Present must be 0.
- CONNACK properties advertise Server limits and capabilities: Session Expiry Interval, Receive Maximum, Maximum QoS, Retain Available, Maximum Packet Size, Assigned Client Identifier, Topic Alias Maximum, Wildcard Subscription Available, Subscription Identifier Available, Shared Subscription Available, Server Keep Alive, etc.

### PUBLISH

- Fixed header: type `3`, flags are DUP, QoS bit 2, QoS bit 1, RETAIN.
- QoS bits: `00` QoS 0, `01` QoS 1, `10` QoS 2, `11` reserved/malformed.
- DUP must be 0 for QoS 0 messages and must be 1 for retransmission of a PUBLISH packet.
- Variable Header fields: Topic Name, Packet Identifier if QoS > 0, Properties.
- Topic Name must be present and UTF-8 encoded unless a valid Topic Alias mapping supplies it; Topic Name in PUBLISH must not contain wildcard characters.
- PUBLISH properties include Payload Format Indicator, Message Expiry Interval, Topic Alias, Response Topic, Correlation Data, User Property, Subscription Identifier, and Content Type. Client-to-Server PUBLISH must not contain Subscription Identifier.
- Expected response: QoS 0 none, QoS 1 PUBACK, QoS 2 PUBREC.
- Topic Alias mappings are per Network Connection and are independent in each direction; alias 0 is invalid; aliases must be within the negotiated Topic Alias Maximum.
- Receive Maximum limits unacknowledged QoS 1/2 PUBLISH packets in flight; it does not limit QoS 0.

### PUBACK, PUBREC, PUBREL, PUBCOMP

| Packet | Type | Flags | Variable Header | Reason Code default when omitted | Payload |
|---|---:|---|---|---|---|
| PUBACK | 4 | `0000` | Packet Identifier, optional Reason Code, optional Properties | `0x00` if Remaining Length = 2 | None |
| PUBREC | 5 | `0000` | Packet Identifier, optional Reason Code, optional Properties | `0x00` if Remaining Length = 2 | None |
| PUBREL | 6 | `0010` | Packet Identifier, optional Reason Code, optional Properties | `0x00` if Remaining Length = 2 | None |
| PUBCOMP | 7 | `0000` | Packet Identifier, optional Reason Code, optional Properties | `0x00` if Remaining Length = 2 | None |

Reason String and User Property may be included only if they do not cause the packet to exceed the receiver’s Maximum Packet Size. Allowed reason codes are packet-specific and must come from the corresponding reason-code set.

### SUBSCRIBE and SUBACK

- SUBSCRIBE fixed header: type `8`, flags `0010`. Variable Header: Packet Identifier, Properties. Payload: one or more Topic Filter + Subscription Options pairs.
- SUBSCRIBE properties: Subscription Identifier, User Property. Subscription Identifier value must be 1 to 268,435,455.
- Subscription Options byte: bits 0-1 Maximum QoS; bit 2 No Local; bit 3 Retain As Published; bits 4-5 Retain Handling; bits 6-7 reserved and must be 0. QoS value 3 is Protocol Error. No Local = 1 on Shared Subscription is Protocol Error.
- SUBACK fixed header: type `9`, flags `0000`. Variable Header: Packet Identifier, Properties. Payload: one reason code per Topic Filter in the same order.
- SUBACK reason codes: `0x00`, `0x01`, `0x02`, `0x80`, `0x83`, `0x87`, `0x8F`, `0x91`, `0x97`, `0x9E`, `0xA1`, `0xA2`.

### UNSUBSCRIBE and UNSUBACK

- UNSUBSCRIBE fixed header: type `10`, flags `0010`. Variable Header: Packet Identifier, Properties. Payload: at least one UTF-8 Topic Filter.
- UNSUBACK fixed header: type `11`, flags `0000`. Variable Header: Packet Identifier, Properties. Payload: one reason code per Topic Filter in the same order.
- UNSUBACK reason codes: `0x00`, `0x11`, `0x80`, `0x83`, `0x87`, `0x8F`, `0x91`.

### PINGREQ and PINGRESP

- PINGREQ: type `12`, flags `0000`, Remaining Length 0, no Variable Header, no Payload.
- PINGRESP: type `13`, flags `0000`, Remaining Length 0, no Variable Header, no Payload.
- Server must send PINGRESP in response to PINGREQ. Client should close the Network Connection if it does not receive PINGRESP in a reasonable time after PINGREQ.

### DISCONNECT

- DISCONNECT: type `14`, flags `0000`. It can be sent by Client or Server. Server must not send DISCONNECT until after it has sent CONNACK with Reason Code less than `0x80`.
- Variable Header: optional Disconnect Reason Code and Properties. If Remaining Length is 0, Reason Code defaults to `0x00` and there are no properties. If Reason Code is `0x80` or greater, the Network Connection must be closed whether or not the packet is sent.
- DISCONNECT properties: Session Expiry Interval, Reason String, User Property, Server Reference. Server must not send Session Expiry Interval in DISCONNECT.
- After sending DISCONNECT, the sender must not send further MQTT Control Packets on that Network Connection and must close it.

### AUTH

- AUTH: type `15`, flags `0000`, no Payload. Variable Header: Authenticate Reason Code and Properties.
- AUTH reason codes: `0x00` Success, `0x18` Continue authentication, `0x19` Re-authenticate.
- AUTH properties: Authentication Method, Authentication Data, Reason String, User Property. If enhanced authentication was not negotiated via CONNECT Authentication Method, the Server must not send AUTH and the Client must not send AUTH.

## 7. Topic, subscription, flow-control, and error-handling context

- Topic Filters may contain wildcards; Topic Names in PUBLISH and Response Topic must not contain wildcards.
- `/` separates topic levels. Adjacent separators create zero-length levels. Topic Names and Topic Filters are case-sensitive, can contain spaces, can begin/end with `/`, and must be at least one character long except a PUBLISH Topic Name can be zero length only when a valid Topic Alias supplies the topic.
- `#` is a multi-level wildcard and must either be alone or follow a topic level separator, and must be the final character of the Topic Filter.
- `+` is a single-level wildcard and must occupy an entire topic level.
- Topic Filters beginning with `#` or `+` do not match Topic Names beginning with `$`; Server uses `$` topics for Server-defined purposes such as `$SYS/`.
- Server must not normalize Topic Names or Topic Filters during matching; matching is character-for-character for non-wildcarded levels.
- Shared Subscription Topic Filters start with `$share/{ShareName}/` followed by a normal Topic Filter. ShareName must not include `/`, `+`, or `#`; the Topic Filter part follows normal wildcard rules.
- Flow control: Receive Maximum applies to unacknowledged QoS 1 and QoS 2 PUBLISH packets. The sender’s quota is decremented when sending QoS>0 PUBLISH and incremented when receiving PUBACK, PUBCOMP, or PUBREC with Reason Code >= `0x80`. Other MQTT Control Packets must still be processed even if the quota is zero.
- Error handling: Malformed Packet means the packet cannot be parsed; Protocol Error means parsed data is not allowed by protocol or session state. On server-detected Malformed Packet or Protocol Error with specified Reason Code, the Server must close the Network Connection. For CONNECT it may send CONNACK first; for other packets it should send DISCONNECT first.

## 8. WebSocket transport context

- MQTT Control Packets over WebSocket must be in binary data frames.
- A single WebSocket binary frame can contain multiple MQTT Control Packets or partial MQTT Control Packets; do not assume packet/frame alignment.
- Client must offer WebSocket subprotocol `mqtt`; Server-selected subprotocol must be `mqtt`.
- WebSocket URI does not affect MQTT protocol semantics.

## 9. Validation-critical Protocol Error / Malformed Packet checklist

MQTT 5.0 uses many rules that are validation-critical even when they are phrased as “Protocol Error” or “Malformed Packet” rather than as standalone numbered `MUST` statements. Keep these checks with the numbered requirements.

### 9.1 Generic packet and property errors

- A packet that cannot be parsed according to MQTT 5.0 is a **Malformed Packet**.
- A parsed packet containing data that is not allowed by the protocol, or inconsistent with Client/Server state, is a **Protocol Error**.
- Invalid fixed-header reserved bits are Malformed Packet / Protocol Error according to the packet-specific rule.
- A Property Identifier not valid for the packet type is a Malformed Packet.
- A Property value not encoded with the property’s specified data type is a Malformed Packet.
- Property Length must be present for packets/properties that carry properties; if there are no properties, Property Length is `0`.
- Repeated properties are Protocol Errors unless the property is explicitly allowed to appear multiple times. User Property is repeatable. Subscription Identifier can appear multiple times in PUBLISH when the publication matches multiple subscriptions; SUBSCRIBE’s Subscription Identifier property is not repeatable.
- Any packet exceeding the peer’s Maximum Packet Size is a Protocol Error; use Reason Code `0x95` where applicable.
- Reason String and User Property must not be sent if doing so would exceed the receiver’s Maximum Packet Size.

### 9.2 CONNECT and Will-related property errors

- CONNECT reserved flag bit 0 must be `0`; otherwise the CONNECT is malformed.
- Will QoS value `3` is a Malformed Packet.
- Session Expiry Interval must not appear more than once.
- Receive Maximum must not appear more than once and must not be `0`.
- Maximum Packet Size must not appear more than once and must not be `0`.
- Topic Alias Maximum must not appear more than once.
- Request Response Information must not appear more than once and must be `0` or `1`.
- Request Problem Information must not appear more than once and must be `0` or `1`.
- Authentication Method must not appear more than once.
- Authentication Data must not appear without Authentication Method and must not appear more than once.
- Will Delay Interval, Payload Format Indicator, Message Expiry Interval, Content Type, Response Topic, and Correlation Data must not appear more than once in Will Properties.
- Will Payload with Payload Format Indicator `1` is expected to be well-formed UTF-8 if validated.

### 9.3 CONNACK property and capability errors

- Connect Acknowledge Flags bits 7-1 must be `0`.
- If CONNACK Reason Code is `0x80` or greater, Session Present must be `0` and the Network Connection must be closed.
- CONNACK Connect Reason Code must be one of the allowed Connect Reason Code values.
- CONNACK Receive Maximum must not appear more than once and must not be `0`.
- Maximum QoS must not appear more than once and must be `0` or `1`.
- Retain Available, Wildcard Subscription Available, Subscription Identifier Available, and Shared Subscription Available must not appear more than once and must be `0` or `1`.
- Maximum Packet Size must not appear more than once and must not be `0`.
- Assigned Client Identifier, Topic Alias Maximum, Reason String, Server Keep Alive, Response Information, Server Reference, Authentication Method, and Authentication Data must not appear more than once.
- A client must not use a server-declared unavailable feature: Retain Available `0`, Maximum QoS lower than requested publish QoS, Wildcard Subscription Available `0`, Subscription Identifier Available `0`, Shared Subscription Available `0`, Topic Alias Maximum `0` or absent.

### 9.4 PUBLISH errors and edge cases

- QoS bits `11` are a Malformed Packet.
- A PUBLISH with QoS 0 must not contain a Packet Identifier.
- A PUBLISH with QoS 1 or 2 must contain a non-zero Packet Identifier.
- Topic Name must not contain wildcards.
- Topic Name can be zero length only when a valid Topic Alias mapping supplies the Topic Name.
- Topic Alias must not appear more than once, must not be `0`, must not exceed the receiver’s negotiated Topic Alias Maximum, and cannot be used with zero-length Topic Name before a mapping is established.
- Topic Alias mappings are scoped to one Network Connection and must not be carried forward across connections.
- Response Topic must not appear more than once, must be UTF-8, and must not contain wildcards.
- Correlation Data, Payload Format Indicator, Message Expiry Interval, and Content Type must not appear more than once.
- Subscription Identifier value must not be `0`; a PUBLISH sent by a Client to a Server must not contain a Subscription Identifier.
- A received PUBLISH with a Subscription Identifier other than one caused by the matching SUBSCRIBE is a Protocol Error.
- If Payload Format Indicator is `1`, the payload is indicated as UTF-8 Encoded Character Data; receivers may validate and reject with `0x99` if invalid.

### 9.5 QoS acknowledgement and flow-control errors

- PUBACK, PUBREC, PUBREL, and PUBCOMP must use reason codes allowed for that packet.
- PUBREL fixed-header flags must be `0010`; otherwise it is malformed.
- PUBREL and PUBCOMP Reason String and User Property must not appear more than once where applicable.
- Receive Maximum applies to unacknowledged QoS 1 and QoS 2 PUBLISH packets only. Exceeding it is a Protocol Error; use `0x93` where applicable.
- When PUBACK or PUBREC has Reason Code `0x80` or greater, the corresponding PUBLISH is treated as acknowledged and must not be retransmitted.

### 9.6 SUBSCRIBE / SUBACK errors

- SUBSCRIBE fixed-header flags must be `0010`; otherwise it is malformed.
- SUBSCRIBE payload must contain at least one Topic Filter plus Subscription Options pair.
- Topic Filters must be UTF-8 Encoded Strings.
- Subscription Options reserved bits must be `0`.
- Maximum QoS value `3` is a Protocol Error.
- No Local bit set to `1` on a Shared Subscription is a Protocol Error.
- Retain Handling value `3` is a Protocol Error.
- SUBSCRIBE Subscription Identifier must not be `0` and must not appear more than once.
- SUBACK payload reason-code order must match SUBSCRIBE Topic Filter order.
- SUBACK must contain one reason code per Topic Filter / Subscription Options pair.

### 9.7 UNSUBSCRIBE / UNSUBACK errors

- UNSUBSCRIBE fixed-header flags must be `0010`; otherwise it is malformed.
- UNSUBSCRIBE payload must contain at least one Topic Filter.
- UNSUBSCRIBE Topic Filters must be UTF-8 Encoded Strings.
- UNSUBACK payload reason-code order must match UNSUBSCRIBE Topic Filter order.
- UNSUBACK must contain one reason code per Topic Filter.

### 9.8 DISCONNECT and AUTH errors

- DISCONNECT fixed-header flags must be `0000`; reserved bits must be zero.
- A Server must not send DISCONNECT before sending CONNACK with a Reason Code less than `0x80`.
- A Server must not send Session Expiry Interval in DISCONNECT.
- If the CONNECT Session Expiry Interval was `0`, a Client must not set a non-zero Session Expiry Interval in DISCONNECT.
- After sending DISCONNECT, the sender must not send any more MQTT Control Packets on that Network Connection and must close the Network Connection.
- AUTH fixed-header flags must be `0000`; otherwise it is malformed.
- AUTH must not be sent unless the CONNECT used the same Authentication Method.
- AUTH Authentication Data and Reason String must not appear more than once.
- If CONNECT did not include Authentication Method, the Server must not send AUTH or Authentication Method in CONNACK, and the Client must not send AUTH.

## 10. Mandatory normative statements

The following table contains the numbered MQTT 5.0 mandatory normative statements extracted from Appendix B, with whitespace, `<br>` line breaks, and obvious parser artifacts cleaned. Three parser-sensitive statements were corrected against the main-body text: CONNECT payload field order, Topic Alias Maximum absent/zero target, and Will User Property ordering.
...
| [MQTT-3.1.2-27] | If Topic Alias Maximum is absent or zero, the Server MUST NOT send any Topic Aliases to the Client. |
...
| [MQTT-3.1.3-1] | The Payload of the CONNECT packet contains one or more length-prefixed fields, whose presence is determined by the flags in the Variable Header. These fields, if present, MUST appear in the order Client Identifier, Will Properties, Will Topic, Will Payload, User Name, Password. |
...
| [MQTT-3.1.3-10] | The Server MUST maintain the order of User Properties when publishing the Will Message. |
