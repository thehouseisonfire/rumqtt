# MQTT 3.1.1 — LLM Validation Corpus (Requirements + Required Context)

This version is intended for systematic validation of an MQTT 3.1.1 client implementation. It keeps the numbered normative requirements and adds back the compact context needed to interpret them: byte layouts, flag values, packet type values, payload/Packet Identifier rules, return codes, topic matching rules, and conformance scope.

## Minimum validation context

### Conformance scope for a client

A conformant MQTT Client must:

- send Control Packets whose format matches MQTT 3.1.1 packet format rules;
- satisfy all applicable MUST-level requirements in Chapters 1, 2, 3, 4, Chapter 6 if WebSocket is used, and Chapter 7;
- support at least one ordered, lossless, bidirectional byte-stream transport;
- not require non-standard extensions to interoperate with other conformant implementations.

When validating a client, include both outbound behavior (packets the client sends) and inbound behavior (packets the client receives and must acknowledge, reject, or close on).
...
- Bits in a byte are numbered 7 through 0. Bit 7 is the most significant bit; bit 0 is the least significant bit.
- 16-bit integer values are encoded big-endian: MSB then LSB.
- UTF-8 strings are encoded as: 2-byte big-endian byte length, followed by that many UTF-8 bytes. Maximum encoded string length is 65,535 bytes.
- UTF-8 string data must be well-formed; must not contain U+0000; U+FEFF encoded as `0xEF 0xBB 0xBF` must be treated as the character U+FEFF and not stripped.

### MQTT Control Packet structure

Every MQTT Control Packet consists of:

1. Fixed header, always present.
2. Variable header, present for some packet types.
3. Payload, present for some packet types.

Fixed header byte 1:

- bits 7-4: MQTT Control Packet type;
- bits 3-0: packet-type-specific flags.

Remaining Length starts at byte 2 and gives the number of bytes after the Remaining Length field, including variable header and payload, but excluding the bytes used to encode Remaining Length.

### Remaining Length encoding

Remaining Length uses 1 to 4 bytes. Each byte contributes 7 data bits; bit 7 is a continuation bit.
...
- Decode little-endian base-128 groups: `(byte & 127) * multiplier`, multiplier starts at 1 and multiplies by 128 per byte.
- Continue while `(byte & 128) != 0`.
- More than 4 encoded bytes is malformed.
- Maximum valid Remaining Length is 268,435,455.

### Control Packet types and fixed-header flags

| Type | Packet | Direction | Fixed-header flags bits 3-0 |
|---:|---|---|---|
...
| 15 | Reserved | Forbidden | n/a |

Any reserved fixed-header flag value that does not match the required bits is invalid.

### Packet Identifier presence

Packet Identifier is a 2-byte non-zero unsigned integer when present.

| Packet | Packet Identifier present? |
|---|---|
| CONNECT | No |
| CONNACK | No |
| PUBLISH | Yes only if QoS > 0 |
| PUBACK | Yes |
| PUBREC | Yes |
| PUBREL | Yes |
| PUBCOMP | Yes |
...
### CONNECT packet validation context

- Fixed header: type `1`, flags `0000`.
- Remaining Length = 10-byte variable header + payload length.
- Variable header fields, in order:
  1. Protocol Name: UTF-8 string `MQTT`, encoded as `00 04 4D 51 54 54`.
  2. Protocol Level: `0x04` for MQTT 3.1.1.
  3. Connect Flags byte:
  ...
  4. Keep Alive: 2-byte big-endian seconds.
- Payload fields, if present, must appear in this order: Client Identifier, Will Topic, Will Message, User Name, Password.
- Client Identifier is required and first in the payload. It is a UTF-8 string.
- Server must allow ClientIds 1 to 23 UTF-8 encoded bytes using only `0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ`.
- Zero-length ClientId is allowed only if the Server supports it and CleanSession is 1.
- If Will Flag is 0, Will QoS and Will Retain must be 0 and Will Topic/Will Message must be absent.
- If Will Flag is 1, Will Topic and Will Message must be present; Will QoS may be 0, 1, or 2, but not 3.
- If User Name Flag is 0, user name must be absent.
- If Password Flag is 0, password must be absent.
- If User Name Flag is 0, Password Flag must also be 0.
- Client must send PINGREQ when no other Control Packets are sent within Keep Alive.

### CONNACK packet validation context

- Fixed header: type `2`, flags `0000`, Remaining Length `2`.
- Variable header byte 1: Connect Acknowledge Flags. Bits 7-1 reserved and set to 0; bit 0 is Session Present.
- Variable header byte 2: Connect Return Code.

Allowed Connect Return Codes:
...
### PUBLISH packet validation context

- Fixed header: type `3`; flags are DUP, QoS, QoS, RETAIN.
- QoS bits:

| QoS bits | QoS level | Meaning |
|---|---:|---|
| `00` | 0 | At most once |
| `01` | 1 | At least once |
| `10` | 2 | Exactly once |
| `11` | reserved | malformed |

- DUP must be 0 for QoS 0 messages.
- Variable header fields, in order: Topic Name, then Packet Identifier only if QoS > 0.
- Topic Name must be present, UTF-8 encoded, and must not contain wildcards.
- Payload is application-specific and may be zero length.
- Expected response by received QoS:

| Incoming PUBLISH QoS | Expected receiver response |
|---:|---|
| 0 | None |
| 1 | PUBACK |
| 2 | PUBREC |

### PUBACK, PUBREC, PUBREL, PUBCOMP context

| Packet | Type | Fixed-header flags | Remaining Length | Variable header | Payload |
|---|---:|---|---:|---|---|
| PUBACK | 4 | `0000` | 2 | Packet Identifier | None |
| PUBREC | 5 | `0000` | 2 | Packet Identifier | None |
| PUBREL | 6 | `0010` | 2 | Packet Identifier | None |
| PUBCOMP | 7 | `0000` | 2 | Packet Identifier | None |

### SUBSCRIBE and SUBACK context

SUBSCRIBE:

- Fixed header: type `8`, flags `0010`.
- Variable header: Packet Identifier.
- Payload: one or more Topic Filter / Requested QoS pairs.
- Topic Filters are UTF-8 encoded strings.
- Requested QoS byte: upper 6 bits must be 0; lower 2 bits must be 0, 1, or 2.

SUBACK:

- Fixed header: type `9`, flags `0000`.
- Variable header: Packet Identifier from the SUBSCRIBE packet.
- Payload: one return code per requested Topic Filter, in the same order.

Allowed SUBACK return codes:
...
### UNSUBSCRIBE and UNSUBACK context

UNSUBSCRIBE:

- Fixed header: type `10`, flags `0010`.
- Variable header: Packet Identifier.
- Payload: one or more UTF-8 Topic Filters, packed contiguously.

UNSUBACK:

- Fixed header: type `11`, flags `0000`, Remaining Length `2`.
- Variable header: Packet Identifier from the UNSUBSCRIBE packet.
- Payload: none.

### PINGREQ, PINGRESP, DISCONNECT context

| Packet | Type | Direction | Fixed-header flags | Remaining Length | Variable header | Payload |
|---|---:|---|---|---:|---|---|
| PINGREQ | 12 | Client → Server | `0000` | 0 | None | None |
| PINGRESP | 13 | Server → Client | `0000` | 0 | None | None |
| DISCONNECT | 14 | Client → Server | `0000` | 0 | None | None |

After sending DISCONNECT, the client must close the Network Connection and must not send more Control Packets on it.

### QoS protocol validation context

QoS 0:

- Sender sends PUBLISH with QoS=0 and DUP=0.
- Receiver sends no response.

QoS 1:

- Sender assigns unused Packet Identifier, sends PUBLISH QoS=1 DUP=0, and treats it as unacknowledged until PUBACK.
- Receiver responds with PUBACK using the same Packet Identifier.
- After PUBACK is sent, a later PUBLISH with the same Packet Identifier is a new publication regardless of DUP.

QoS 2:

- Sender assigns unused Packet Identifier, sends PUBLISH QoS=2 DUP=0, waits for PUBREC, sends PUBREL with the same Packet Identifier, waits for PUBCOMP, and must not resend PUBLISH after sending PUBREL.
- Receiver sends PUBREC for incoming QoS 2 PUBLISH, repeats PUBREC for duplicate PUBLISH with same Packet Identifier until PUBREL, sends PUBCOMP for PUBREL, and treats a later PUBLISH with the same Packet Identifier as new after PUBCOMP.

Reconnect retry:

- When CleanSession=0 and a client reconnects, both sides resend unacknowledged QoS>0 PUBLISH packets and PUBREL packets using original Packet Identifiers.

Ordering:

- Client resends PUBLISH packets in original send order.
- Client sends PUBACK in receive order for QoS 1.
- Client sends PUBREC in receive order for QoS 2.
- Client sends PUBREL in receive order of corresponding PUBREC packets.

### Topic validation context

- `/` is the topic level separator.
- `#` is a multi-level wildcard. It must be either the entire Topic Filter or be preceded by `/`, and it must be the final character.
- `+` is a single-level wildcard. It must occupy an entire topic level.
- Wildcards may appear in Topic Filters but not in Topic Names.
- Topic Names and Topic Filters are case sensitive.
- Space is allowed.
- Leading and trailing `/` create distinct Topic Names/Filters.
- `/` alone is valid.
- Topic Names and Topic Filters must be at least one character long and must not include U+0000.
- They must be UTF-8 encoded and no more than 65,535 encoded bytes.
- Matching must not normalize, modify, or substitute characters; non-wildcard levels match character-for-character.
- A Topic Filter starting with `#` or `+` must not match Topic Names beginning with `$`.
- To receive both `$` topics and non-`$` topics, a client must subscribe separately, e.g. `#` and `$SYS/#`.

### WebSocket transport context

If MQTT uses WebSocket:

- MQTT Control Packets are sent in WebSocket binary data frames.
- A WebSocket frame may contain multiple or partial MQTT Control Packets; do not assume packet/frame alignment.
- Client offers subprotocol `mqtt`.
- Server-selected subprotocol must be `mqtt`.
- The WebSocket URI does not change MQTT protocol semantics.
...
- **[MQTT-3.1.3-5]** The Server MUST allow ClientIds which are between 1 and 23 UTF-8 encoded bytes in length, and that contain only the characters "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXY Z".
