...
use rumqttc::{
    ConnectionError, Event, EventLoop, MqttOptions, PersistedAckMode, PersistedPublish,
...
async fn read_packet(peer: &mut DuplexStream) -> Packet {
    ...
    Packet::read(&mut bytes, Some(1024)).unwrap()
}
...
async fn failed_initial_attempt_emits_attempt_and_failed_only() {
    ...
    let mut options = MqttOptions::new("trace-failure-v5", "localhost");
    ...
    assert_eq!(attempt.field("protocol"), &Value::Str("v5".to_owned()));
    assert_eq!(failed.field("protocol"), &Value::Str("v5".to_owned()));
...
async fn restored_connection_duplicate_connack_emits_wired_lifecycle_in_order() {
    ...
    let mut options = MqttOptions::new("trace-lifecycle-v5", "localhost");
    options
        .set_clean_start(false)
        .set_session_expiry_interval(Some(SESSION_EXPIRY))
        .set_session_store(MemorySessionStore::new(persisted_qos1_session(
            "trace-lifecycle-v5",
        )));
    ...
    tokio::time::timeout(
        ...
        async move {
            let broker = tokio::spawn(async move {
                ...
                assert!(matches!(read_packet(&mut peer).await, Packet::Connect(..)));
                let connack = [0x20, 0x03, 0x01, 0x00, 0x00];
    ...
    assert_lifecycle_events(&capture.events(), "v5");
}
