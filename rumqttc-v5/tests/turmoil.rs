...
const MAX_PACKET_SIZE: u32 = 10 * 1024;
const PORT: u16 = 1883;
...
async fn read_packet(stream: &mut TcpStream, buffer: &mut BytesMut) -> turmoil::Result<Packet> {
    loop {
        match Packet::read(buffer, Some(MAX_PACKET_SIZE)) {
...
async fn write_packet(stream: &mut TcpStream, packet: Packet) -> turmoil::Result {
    ...
    packet.write(&mut buffer, Some(MAX_PACKET_SIZE))?;
    ...
    loop {
        match read_packet(&mut stream, &mut buffer).await? {
            Packet::Connect(_, _, _) => {
                write_packet(&mut stream, Packet::ConnAck(connack(false))).await?;
            }
            Packet::Subscribe(subscribe) => {
                ...
                write_packet(
                    &mut stream,
                    Packet::Publish(Publish::new(TOPIC, QoS::AtMostOnce, PAYLOAD, None)),
                )
                .await?;
            }
...
async fn happy_path_client() -> turmoil::Result {
    let options = mqtt_options("turmoil-v5-happy-path");
...
fn broker_crash_replays_unacknowledged_qos1_publish() {
    ...
    sim.host("broker", move || {
        ...
        async move {
            ...
            let Packet::Connect(connect, _, _) = read_packet(&mut stream, &mut buffer).await?
            else {
                return Err("broker expected CONNECT".into());
            };
            assert!(!connect.clean_start);
            write_packet(&mut stream, Packet::ConnAck(connack(attempt > 1))).await?;
            ...
            loop {
                match read_packet(&mut stream, &mut buffer).await? {
                    Packet::Publish(publish) => {
                        ...
                        write_packet(&mut stream, Packet::PubAck(PubAck::new(publish.pkid, None)))
                            .await?;
    ...
    sim.client("client", async move {
        let mut options = mqtt_options("turmoil-v5-reconnect");
        options
