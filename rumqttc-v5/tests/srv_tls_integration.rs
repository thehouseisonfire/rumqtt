...
fn certificates() -> Certificates {
    ...
    // macOS Security Framework cannot import the EC PKCS#8 keys generated by rcgen's default
    // algorithm. RSA PKCS#8 identities work across all native-tls backends.
    let generate_key = || KeyPair::generate_for(&PKCS_RSA_SHA256).unwrap();
    let ca = CertifiedIssuer::self_signed(ca_params, generate_key()).unwrap();
