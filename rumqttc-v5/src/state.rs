...
impl MqttState {
    ...
    fn outgoing_publish_with_notice(
        ...
        if publish.qos != QoS::AtMostOnce {
            ...
            if self.outgoing_rel.contains(pkid as usize)
                || self.outgoing_rel_replay.contains(pkid as usize)
                || self.tracked_subscribe.contains_key(&pkid)
                || self.tracked_unsubscribe.contains_key(&pkid)
            {
    ...
    fn outgoing_puback(&mut self, puback: PubAck) -> Result<Packet, StateError> {
        if !self.incoming_puback.contains(puback.pkid as usize) {
            error!("Unsolicited puback request: {:?}", puback.pkid);
            return Err(StateError::Unsolicited(puback.pkid));
        }

        self.incoming_puback.set(puback.pkid as usize, false);
    ...
    fn save_pubrel_with_notice(
        ...
        let pkid_index = pubrel.pkid as usize;
    ...
    fn next_available_auto_topic_alias(&self) -> Option<u16> {
        let next_auto_topic_alias = self.next_auto_topic_alias?;
        (next_auto_topic_alias..=self.broker_topic_alias_max)
            .find(|&alias| !self.outgoing_topic_aliases.contains_key(&alias))
    }
    ...
    fn advance_next_auto_topic_alias(&mut self) {
        let Some(mut alias) = self.next_auto_topic_alias else {
            return;
        };

        while alias <= self.broker_topic_alias_max
            && self.outgoing_topic_aliases.contains_key(&alias)
        {
            let Some(next_alias) = alias.checked_add(1) else {
                self.next_auto_topic_alias = None;
                return;
            };
            alias = next_alias;
        }

        self.next_auto_topic_alias = (alias <= self.broker_topic_alias_max).then_some(alias);
    }
...
mod test {
    ...
    fn next_pkid_returns_none_when_all_publish_packet_identifiers_are_in_use() {
        ...
        for pkid in 1..=4 {
            ...
            mqtt.tracked_subscribe
                .insert(pkid, (Subscribe::new(filter, None), notice));
        }
    ...
    fn outgoing_publish_rejects_packet_identifier_used_by_non_publish_flow() {
        ...
        let (notice, _) = SubscribeNoticeTx::new();
        mqtt.tracked_subscribe
            .insert(3, (Subscribe::new(filter, None), notice));
        ...
        mqtt.tracked_unsubscribe
            .insert(4, (Unsubscribe::new("a/b", None), notice));
    ...
    fn auto_topic_alias_collision_does_not_register_unsent_alias() {
        let mut mqtt = MqttState::builder(2).auto_topic_aliases(true).build();
        mqtt.broker_topic_alias_max = 10;
    ...
    fn auto_topic_aliases_exhaust_without_wrapping_at_u16_max() {
        let mut mqtt = MqttState::builder(u16::MAX)
            .auto_topic_aliases(true)
            .build();
        mqtt.broker_topic_alias_max = u16::MAX;
