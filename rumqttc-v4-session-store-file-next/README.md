# MQTT 3.1.1 session file store

This crate implements `rumqttc-v4-next`'s existing `SessionStore` trait using
...
and configuration compatibility remain the event loop's responsibility.

The namespace is `<configured-root>/v4/`. Version 1 key bytes are exactly:

```text
1:u8 || 4:u8 || scope_length:u32_be || scope UTF-8 bytes ||
