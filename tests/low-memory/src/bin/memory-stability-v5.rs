...
use rumqttc_v5::{
    ...
    QoS,
    mqttbytes::v5::{SubscribeReasonCode, UnsubAckReason},
};
    ...
    async fn poll(&mut self) -> Result<Observation, BoxError> {
        ...
        Ok(match event {
            ...
            Event::Incoming(Packet::UnsubAck(unsuback))
                if unsuback.reasons == [UnsubAckReason::Success] =>
            {
                Observation::Unsubscribed
            }
            Event::Incoming(Packet::UnsubAck(unsuback)) => {
                return Err(format!("UNSUBACK rejected unsubscription: {unsuback:?}").into());
            }
...
async fn main() -> Result<(), BoxError> {
    ...
    println!(
        "profile=memory-stability protocol=v5 warmup_rounds={} measured_rounds={} \
         messages_per_round={} {}",
    ...
    stability::run(&mut harness, "v5", &config).await
}
