//! MQTT 5 file-backed session store.
...
impl From<AdapterError<V5>> for SessionFileStoreError {
    fn from(error: AdapterError<V5>) -> Self {
        match error {
...
/// File-backed MQTT 5 session store using the stable `v5` namespace.
#[derive(Clone, Debug)]
...
/// Encodes a complete MQTT 5 session-store key in the stable version 1 format.
///
/// # Errors
...
pub fn encode_session_store_key(key: &SessionStoreKey) -> Result<Vec<u8>, KeyEncodeError> {
    shared::encode_key::<V5>(key.scope(), key.client_id())
}

/// Returns the stable hashed filename for an MQTT 5 store key.
///
/// # Errors
...
fn box_error(error: SessionFileStoreError) -> rumqttc_v5::SessionStoreError {
    Box::new(error)
}
