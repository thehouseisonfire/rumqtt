...
const ENVELOPE_OVERHEAD: usize = 22;
...
impl rumqttc_v4::SessionStore for MeasuringV4Store {
    ...
    fn save<'a>(
        ...
        let checkpoint_bytes = session
            .encode()
            .map(|bytes| bytes.len() + ENVELOPE_OVERHEAD);
        Box::pin(async move {
...
fn run_growth(args: &GrowthArgs) -> anyhow::Result<()> {
    ...
    for count in COUNTS {
        ...
        rows.push(json!({
            ...
            "envelope_bytes": ENVELOPE_OVERHEAD,
            "total_checkpoint_bytes": canonical + ENVELOPE_OVERHEAD,
        }));
    }
    let mut metrics = BTreeMap::new();
    ...
    metrics.insert(
        ...
        canonical_sizes[4] + ENVELOPE_OVERHEAD as f64,
    );
    let mut samples = BTreeMap::new();
